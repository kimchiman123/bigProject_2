...
# [Refactor] load_data_background removed. Data is now queried on-demand.
...
async def get_items():
    ...
    try:
        # [On-Demand] Query distinct items from DB
        with db_engine.connect() as conn:
            # Get distinct item_name
            result = conn.execute(text("SELECT DISTINCT item_name FROM export_trends ORDER BY item_name")).fetchall()
            csv_items = [row[0] for row in result if row[0]]
...
async def analyze(country: str = Query(...), item: str = Query(...)):
    ...
    # [On-Demand] 데이터베이스에서 직접 조회
    if not db_engine:
         print("❌ [Analyze] Error: Database engine not initialized.", flush=True)
         raise HTTPException(status_code=503, detail="Database connection unavailable.")

    try:
        filtered = pd.DataFrame()
        growth_summary_df = pd.DataFrame() # Local growth matrix context
        
        with db_engine.connect() as conn:
            # Query 1: Specific Item Data (Full Columns for Charts)
            # Fetch essential columns for charts: period_str, export_value, exchange_rate, gdp_level, trend_data
            query_item = text("""
                SELECT * FROM export_trends 
                WHERE country_name = :country AND item_name = :item 
                ORDER BY period_str ASC
            """)
            filtered = pd.read_sql(query_item, conn, params={"country": country_name, "item": csv_item_name})
            
            # Query 2: Country Context for Growth Matrix (All items in this country)
            # Only need columns for growth calculation: item_name, period_str, export_value, export_weight, unit_price
            query_country = text("""
                SELECT item_name, period_str, export_value, export_weight, unit_price 
                FROM export_trends 
                WHERE country_name = :country
            """)
            country_df = pd.read_sql(query_country, conn, params={"country": country_name})
            
            # Calculate Growth Matrix for this country context
            if not country_df.empty:
                # Add temporary country_code column for compatibility with calculation function
                country_df['country_code'] = country_code 
                growth_summary_df = calculate_growth_matrix(country_df)

    except Exception as e:
        print(f"❌ [Analyze] DB Query Error: {e}", flush=True)
    ...
    def get_trend_val(row, key):
        ...
        # JSONB from DB might be dict or str depending on driver? Usually dict with SQLAlchemy + psycopg2
    ...
    if isinstance(first_trend_data, str):
        try: first_trend_data = json.loads(first_trend_data)
        except: pass
    ...
    # ---------------------------------------------------------
    # Use locally calculated growth_summary_df
    country_matrix = growth_summary_df  # Already filtered by country in DB query
